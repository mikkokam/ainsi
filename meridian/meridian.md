---
# Photographs are from Unsplash, used under the Unsplash Licence; each photo's id is in its url.
theme: swiss
logo: assets/meridian.svg
---

<!-- ainsi: layout header -->

<!-- ainsi: prose size=huge -->
# Precision is a process, not a claim

<!-- ainsi: prose color=accent -->
Meridian Instruments · Capital Markets Day, 12 March 2026

![A machine hall in raking light](https://images.unsplash.com/photo-1562045289-8e02d9a3a61f?w=1600&q=80)

---

#### Agenda

<!-- ainsi: agenda -->
1. **The business** what we make and who buys it
2. **The decade** three numbers that moved and one that did not
3. **The plan** where the next hundred million goes
4. **The ask** what we want the market to hold us to

---

<!-- ainsi:layout section -->

# The business

Instruments that measure what nobody else can

---

<!-- ainsi: layout split side=right size=third -->
<!-- ainsi: prose color=accent -->
#### What we make

# Four instruments, one measurement problem

<!-- ainsi: prose size=large -->
Every product in the range answers the same question: how far is this from where it should be, to a tolerance nobody else can hold at production speed.

That is why the range looks narrow from outside and why the moat is real. A competitor can copy the instrument. Copying ==eleven years of calibration data== is a different exercise.

<!-- ainsi: full size=full -->
![A calibration bench under hard light](https://images.unsplash.com/photo-1748399965427-d5630f707096?w=1600&q=80)

---

# The decade in four numbers

<!-- ainsi: figures -->
- **612 M€** revenue, 2025, from 188 M€ in 2016
- **34%** gross margin, unchanged across the decade
- **11** production sites, down from nineteen
- **1** customer lost to a competitor on measurement accuracy

---

<!-- ainsi:layout section tone=soft -->

# The plan

Where the next hundred million goes

---

# Capital allocation to 2029

<!-- ainsi: bar-table -->
| Programme                 | Capital | Share | Payback |
| ---                       | ---     | ---   | ---     |
| Calibration data platform | 44 M€   | 44%   | 3.1 yrs |
| Site consolidation        | 31 M€   | 31%   | 2.4 yrs |
| Optical range extension   | 18 M€   | 18%   | 4.8 yrs |
| Working capital           | 7 M€    | 7%    | n/a     |

---

# The sequence

<!-- ainsi: roadmap -->
| Programme     | 2026      | 2027      | 2028  | 2029 |
| ---           | ---       | ---       | ---   | ---  |
| Data platform | Build     | Build     | Live  |      |
| Consolidation | Two sites | Two sites |       |      |
| Optical range |           | Design    | Pilot | Ship |
| Buyback       | x         | x         | x     | x    |

---

<!-- ainsi: prose size=small -->
# Complex roadmap, with dependencies

<!-- ainsi: prose size=large -->
```mermaid
gantt
    dateFormat YYYY-MM-DD
    axisFormat %Y
    tickInterval 1year
    section Data platform
    Two sites live      :done,   dp1, 2026-01-01, 300d
    Remaining six       :active, dp2, after dp1, 420d
    Cutover             :crit,   dp3, after dp2, 120d
    Platform live       :milestone, m1, after dp3, 0d
    section Consolidation
    Kaunas and Porto    :done,   c1,  2026-02-01, 330d
    Third site          :active, c2,  after c1, 300d
    section Optical range
    Design              :        o1,  after dp2, 240d
    Pilot line          :        o2,  after o1, 210d
    First shipment      :milestone, m2, after o2, 0d
    Ramp                :        o3,  after o2, 300d
    section Capital
    Buyback tranches    :        b1,  2026-01-01, 1460d
```

<!-- ainsi: prose size=small color=soft -->
The grid opposite is four programmes at the resolution a board reads. This is the same plan where it has to carry dependencies, a critical path and two milestones: for that, ask your agent for a mermaid gantt, written in the deck and drawn in the theme.

---

<!-- ainsi: layout split side=left size=half -->
#### The risk we are asked about most

# If the data platform slips, the margin story slips with it

<!-- ainsi: columns -->
- It is one programme carrying nearly half the capital and most of the case. We have said so rather than spreading it across four line items that would each look survivable.
- The mitigation is that the platform ships in three parts and the first is already in production at two sites. A slip moves the third part, not the first.

<!-- ainsi: full size=full -->
![A control room at night](https://images.unsplash.com/photo-1630087220212-f6d16e857b5a?w=1600&q=80)

---

# What we want to be held to

<!-- ainsi: boxes -->
1. **Revenue** 780 M€ to 820 M€ by 2029, organic
2. **Gross margin** 34% or better, every year, no exceptions
3. **Sites** seven by 2028, and we will name them
4. **Buyback** 25 M€ a year while the leverage stays under 1.5x

<!-- ainsi: end -->

> [!IMPORTANT]
> If gross margin goes under 34% in any full year to 2029, the buyback stops before the guidance changes.

---

<!-- ainsi: layout section tone=inverse -->

# Questions

ir@meridian.example
