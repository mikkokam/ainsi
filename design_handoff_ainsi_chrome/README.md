...
- **Block toolbar**, floating above the selected block: `--ac-glass` at `blur(28px)`,
  `--ac-r-panel`, `1px solid rgb(255 255 255 / 12%)`, `--ac-shadow-float`, `padding: 5px 6px`,
  `gap: 3px`. Contents left to right: block-type dropdown (active style: `--ac-blue-wash` fill,
  `--ac-blue-edge` border, `#A8C8FB` text) · divider · four format icons · divider · `size`
  label + four S/M/L/X toggles · divider · `align` label + three toggles · divider · `colour`
  label + three swatches · divider · move-down · delete.
  - Group labels (`size`, `align`, `colour`) are `--ac-text-key` `--ac-ink-3`. Dividers are
    `1px × 18px` `rgb(255 255 255 / 12%)` with `5px` side margins.
  - Active toggle: `--ac-blue-wash` fill + `inset 0 0 0 1px --ac-blue-edge` + `#A8C8FB` icon.
    Inactive: transparent, `#A8A8B0`. Hover: `--ac-hover`, `--ac-ink`.
  - Selected colour swatch keeps its own fill and takes `--ac-focus` as a ring — never a blue fill.
  - Delete is the only destructive control: `--ac-danger` at rest, `--ac-danger-ink` on a
    `rgb(224 139 132 / 16%)` hover fill.
...
- `Ainsi Landing.dc.html` — the design reference. Four turns, newest at top:
  4 identity · 3 system + editor + keyboard · 2 dark shell · 1 first exploration (light,
  superseded — kept for history only, do not build from turn 1).
