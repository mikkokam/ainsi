---
# Photographs are from Unsplash, used under the Unsplash Licence; each photo's id is in its url.
theme: default
logo: assets/lehto-mark.svg
coverLogo: assets/lehto-mark-white.svg
---

<!-- ainsi: layout header -->

<!-- ainsi: prose size=large -->
# Lehto & Mänty

<!-- ainsi: prose color=accent -->
**Bespoke woodwork from Åbo, Finland, since 1928**

![The bench room in morning light, shavings on the floor](https://images.unsplash.com/photo-1547609434-b732edfee020?q=80&w=1600&auto=format&fit=crop)

---

<!-- ainsi: prose color=accent -->
# Wood runs in family

<!-- ainsi: agenda -->
1. **The workshop** four benches, one kiln, and what that limits us to
2. **The wood** what each species does after it leaves us
3. **How a commission runs** brief to delivery, and where it goes wrong
4. **What it costs** and what a smaller budget should buy

---

<!-- ainsi:layout section -->

# The workshop

Four benches, one kiln, sixty pieces a year

---

<!-- ainsi: layout split side=left size=third -->
# What a machine cannot

A dining table with a mitred frame and drawbored joints takes a joiner eleven days.
The same table, dowelled and screwed, takes a factory eleven minutes.
Both will be in the room for a year; only one will still be in use in twenty.

So the question we ask first is how long the piece has to last, and the second is what the room does to it.
Underfloor heating and a sixteen-metre window are harder on a table than any customer.

> [!WARNING]
> Every piece leaves the kiln at 7% moisture, cut for a Finnish winter indoors.
A piece shipped to a coastal house in Spain is made to a different number.

![A joiner cutting a mortise by hand](https://images.unsplash.com/photo-1617873228868-f64a54e91a01?q=80&w=1200&auto=format&fit=crop)

---

# The workshop in four numbers

<!-- ainsi: figures -->
- **98** years in the same building, Lehto since 1928 and Mänty since 1954
- **60** pieces a year, which is the ceiling of four joiners and one kiln
- **7%** moisture at delivery, measured on the piece
- **25 yr** structural guarantee on every joint we cut

---

# Ninety-eight years on the same street

<!-- ainsi: timeline -->
- **1928** Esekiel Lehto rents the ground floor on Kyrkovägen and cuts church furniture.
- **1954** Väinö Mänty arrives with a veneer press, and the name takes its ampersand.
- **1978** The kiln is built in the yard, and the workshop stops buying timber it cannot dry itself.
- **1999** Rakel Lehto starts as an apprentice at the bench she runs now.

---

# Four pieces from this year

<!-- ainsi: tiles columns=4 -->
- ![A dining table](https://images.unsplash.com/photo-1719899913493-1e508c3833e8?q=80&w=900&auto=format&fit=crop) Dining table
- ![A wall of fitted cabinetry](https://images.unsplash.com/photo-1762815990312-5d77b1e907f5?q=80&w=900&auto=format&fit=crop) Fitted wall, birch
- ![Woodwork](https://images.unsplash.com/photo-1572293070287-cc6e7bade025?q=80&w=900&auto=format&fit=crop) Woodwork for new museum in Utsjoki
- ![A proto chair](https://images.unsplash.com/photo-1687068385765-01520ad1a599?q=80&w=900&auto=format&fit=crop) Chair, sixth prototype


---

# What each species does after it leaves us

<!-- ainsi: bar-table -->
| Species     | Hardness | Seasonal movement | Lead time | Price index |
| ----------- | -------- | ----------------- | --------- | ----------- |
| Finnish oak | 1 290    | 2.9%              | 14 weeks  | 100         |
| Ash         | 1 320    | 3.4%              | 10 weeks  | 78          |
| Birch       | 1 210    | 4.1%              | 8 weeks   | 61          |
| Pine, aged  | 380      | 4.8%              | 6 weeks   | 42          |

Movement is the column that decides a design. Ash is harder than oak and cheaper, and it moves half a percent more, which is a drawer that binds in February and a joint we have to cut differently.

---

# Workshop and factory, honestly

<!-- ainsi: comparison -->
|              | A workshop piece                    | A factory piece                  |
| ------------ | ----------------------------------- | -------------------------------- |
| Joints       | Cut to the board in front of us      | Cut to a tolerance, then glued   |
| Repair       | Any part, by us, in twenty years     | The part is not made any more    |
| Lead time    | Eight to sixteen weeks               | From stock, or six weeks         |
| Price        | Two to four times                    | The reason most rooms have one   |

---

<!-- ainsi:layout default tone=soft -->

# The mistake that costs the most

<!-- ainsi: prose size=small -->
A specification that gives a width, a length and a species, and says nothing about the room. We have delivered into an apartment with underfloor heating at 24 degrees and 22% humidity in February, which is drier than the kiln, and a 3.2 metre top will find that out within a month.

The fix is one paragraph in the brief: the heating, the glazing, and whether anyone opens a window. It changes the joint, not the price. A frame-and-panel top moves at the panel and stays flat; a solid slab in that room is a warranty claim we have to honour.

---

# What a finish is actually for

<!-- ainsi: striped-table -->
| Finish        | Wear      | Repairable at home | Refresh   | Feel      |
| ------------- | --------- | ------------------ | --------- | --------- |
| Hardwax oil   | Good      | Yes                | 2 years   | Wood      |
| Soap          | Fair      | Yes                | 6 months  | Raw wood  |
| Lacquer, matt | Very good | No                 | Never     | Sealed    |
| Wax only      | Poor      | Yes                | 3 months  | Wood      |

---

# Three commissions and what each taught

<!-- ainsi: boxes -->
1. **A library in Kirkkonummi** 26 running metres, and the reason we now measure a room twice a year before we cut
2. **A conference table, 4.4 m** it went in through a window, which is a crane and a permit and a line in the quote
3. **A chair, six prototypes** the piece we lost money on, and the one the workshop is now known for

---

<!-- ainsi:layout section tone=ground -->

# How a commission runs

Sixteen weeks, four decisions, one of them yours alone

---

# Where a smaller budget should go

<!-- ainsi: matrix x="what it costs" y="what it is worth in twenty years" -->
- **Solid wood where hands touch it** the top, the arms, the drawer fronts
- **Joints cut for the room** an afternoon of drawing, before anything is cut
- **A rarer species** doubles the material and changes nothing structural
- **Carving and inlay** beautiful, and the first thing a later owner paints over

---

# Brief to delivery

<!-- ainsi: columns -->
1. **Visit** Room measured, heating and glazing recorded, drawings agreed.
2. **Prep** Boards chosen with you in the timber store, grain marked for the top.
3. **Cut** Joints cut, dry-fitted, and photographed before glue.
4. **Install** Delivered, levelled, and the care card left on the piece.

---

# A full interior commission

<!-- ainsi: roadmap -->
|                   | Jan          | Feb | Mar | Apr        | May       |
| ----------------- | ------------ | --- | --- | ---------- | --------- |
| Drawings          | Agreed 14    |     |     |            |           |
| Timber&nbsp;      | Selected     | In the kiln | x |          |           |
| Bench work        |              |     | x   | x          |           |
| Finish and cure   |              |     |     | Oil, 3 coats | Delivery 6 |

---

> A joint you can see is a joint you meant. Everything else is furniture that hopes nobody looks underneath.

<!-- ainsi: prose align=right -->
— Rakel Lehto, granddaughter of the founder, Esekiel Lehto - at the bench since 1999

---

<!-- ainsi: layout header -->

<!-- ainsi: prose size=large align=center -->
# Come and stand in the timber store

<!-- ainsi: prose align=center color=accent -->
Kyrkovägen 1, Åbo. Most Thursdays, and the kettle is always on.

![Lehto & Mänty](https://images.unsplash.com/photo-1772207896656-4210003d65ee?q=80&w=1600&auto=format&fit=crop)
