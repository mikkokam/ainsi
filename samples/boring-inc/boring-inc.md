---
# The boring theme carries a default logo, so this deck names none. A deck with its own swaps
# both marks from here: logo: assets/mark.svg, coverLogo: assets/mark-white.svg.
# Photographs are from Unsplash, used under the Unsplash Licence; each photo's id is in its url.
theme: boring
---

<!-- ainsi: layout header -->

# Operating Review Q3 2026

<!-- ainsi: prose size=small -->
Prepared by Corporate Planning · 8 September 2026

Boring Inc. Group Performance and Outlook

---

# Agenda

<!-- ainsi: agenda -->
1. **Where we stand** the quarter against plan
2. **The three units** revenue, margin, and what moved
3. **Cost programme** what has landed and what has not
4. **Outlook** the guidance we are asking the board to confirm

---

<!-- ainsi:layout section -->

# Where we stand

Revenue ahead of plan, margin behind it

---

# The quarter in four numbers

<!-- ainsi: figures -->
- **184.2 M€** group revenue, up 6.1% on Q3 2025
- **11.4%** operating margin, 60 basis points below plan
- **21.0 M€** operating profit, flat year on year
- **94 days** cash conversion cycle, four days worse than Q2

---

<!-- ainsi: layout split side=right size=two-thirds tone=soft -->
<!-- ainsi: prose size=small -->
# Growth is real, and it is being bought

<!-- ainsi: prose size=small -->
Volume carried the quarter. Price did not: the average contract value fell 2.3% as the renewals signed in 2024 came up against a market that has three more credible vendors in it than it had then.

<!-- ainsi: prose size=small -->
Margin went where the discount went. Nothing in the cost base deteriorated, which is the one piece of good news in the sentence.

> [!CAUTION]
> A further 1.100 contracts renew before March. At Q3 discounting 4.8 M€ of annual revenue at risk.

<!-- ainsi: full size=full -->
![Harbour](https://images.unsplash.com/photo-1565167808479-08dcdbb1d6d7?w=1600&q=80)

---

# The three units

<!-- ainsi: bar-table text=large -->
| Unit | Revenue Q3 | Growth | Margin |
| --- | --- | --- | --- |
| Industrial Services | 82.4 M€ | 4.1% | 14.2% |
| Logistics | 61.8 M€ | 11.7% | 8.9% |
| Facilities | 40.0 M€ | 1.2% | 10.6% |

---

<!-- ainsi: layout split side=left size=half -->
# Logistics is the story

Eleven per cent growth on a book that was shrinking two years ago, and it came from two contracts rather than from the market. Both renew in 2028.

The margin is the problem. Sub-nine per cent in a unit that plans at twelve means the growth is being delivered by subcontractors at rates negotiated when we had no alternative.

Insourcing the Turku hub is the single largest margin action available to us this year. It costs 3.2 M€ and returns it in fourteen months.

<!-- ainsi: full size=full -->
![Pallets and racking in a distribution warehouse](https://images.unsplash.com/photo-1553413077-190dd305871c?w=1600&q=80)

---

<!-- ainsi: layout section tone=inverse -->

# Cost programme

Fourteen of twenty-two actions complete

---

# What has landed

<!-- ainsi: boxes -->
1. **Procurement consolidation** 4.1 M€ annualised, ahead of the 3.5 M€ target
2. **Property footprint** three sites closed, 1.8 M€, one lease still in dispute
3. **Contractor rates** renegotiated across Industrial Services, 2.2 M€
4. **IT estate** flat, and the migration slipped a quarter

---

# What has not

<!-- ainsi: striped-table -->
| Action | Planned | Actual | Status |
| --- | --- | --- | --- |
| ERP consolidation | Q2 2026 | Q1 2027 | Vendor resourcing |
| Shared service centre | Q3 2026 | Q4 2026 | Hiring behind |
| Fleet renewal | Q3 2026 | Q3 2026 | Delivered |

---

# The plan to March

<!-- ainsi: roadmap -->
| Workstream | Q4 2026 | Q1 2027 | Q2 2027 |
| --- | --- | --- | --- |
| Turku insourcing | Build | Cut over | |
| ERP consolidation | | Migration | Stabilise |
| Renewals defence | Pricing floor | Pricing floor | |
| Shared services | Hire | Hire | Live |

---

# What we are asking the board to confirm

<!-- ainsi: timeline -->
1. **Guidance held** full-year revenue 735 M€ to 745 M€, margin 11.5% to 12.0%
2. **Turku insourcing approved** 3.2 M€ capital, payback fourteen months
3. **Pricing floor** no renewal below 92% of list without unit head sign-off

&nbsp;


> [!IMPORTANT]
> Without the pricing floor, the margin guidance is not defensible past Q1.

---

<!-- ainsi: layout section -->

# Questions

Corporate Planning · planning@boring.example
